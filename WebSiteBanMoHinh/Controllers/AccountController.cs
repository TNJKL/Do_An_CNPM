...
namespace WebSiteBanMoHinh.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> CancelOrder(string ordercode)
        {
            ...
            if (order.Status == 0 || order.Status == 5 || order.Status == 6 || order.Status == 7)
            {
                return BadRequest("Không thể hủy vì đơn hàng đã được xử lý, đang chuẩn bị, đang giao hoặc đã giao thành công.");
            }
